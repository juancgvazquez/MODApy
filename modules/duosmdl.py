...
def duos(vcf1, vcf2):
	...
	return df_final
	#vcfmgr.df_to_excel(df_final, cfg.resultsPath + 'Duos/' + ':'.join([vcf1df.name, vcf2df.name]) + '.xlsx')
...
def trios(vcf1, vcf2, vcf3):
	...
	return df_final
	#vcfmgr.df_to_excel(df_final, cfg.resultsPath + 'Trios/' + ':'.join([vcf1df.name, vcf2df.name, vcf3df.name]) + '.xlsx')
	#vcfmgr.df_to_vcf(df_final, cfg.resultsPath + 'Trios/' + ':'.join([vcf1df.name, vcf2df.name, vcf3df.name]) + '.vcf')