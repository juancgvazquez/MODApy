...
library(shinyFiles)
# python config -------------------------------------------------------------------
modapydir <- "/Users/juan.vazquez/dev/charly/MODApy/MODApy/"
cfgpath <- paste0(modapydir, "config.ini")
logfile <- paste0(modapydir, "logs/current_run.log")
...
getcommand <- function(input) {
  ...
  switch(input$tabset,
    ...
    Duos = {
      ...
      venn <- ""
      if (input$vennplaceD == input$Patient1D) {
        venn <- "A"
      } else if (input$vennplaceD == input$Patient2D) {
        venn <- "B"
      } else if (input$vennplaceD == paste(input$Patient1D, input$Patient2D, sep = ":")) {
        venn <- "A:B"
      }
      if (venn != "") {
    ...
    Trios = {
      ...
      if (input$vennplaceT == input$Patient1T) {
        venn <- "A"
      } else if (input$vennplaceT == input$Patient2T) {
        venn <- "B"
      } else if (input$vennplaceT == input$Patient3T) {
        venn <- "C"
      } else if (input$vennplaceT == paste(input$Patient1T, input$Patient2T, sep = ":")) {
        venn <- "A:B"
      } else if (input$vennplaceT == paste(input$Patient1T, input$Patient3T, sep = ":")) {
        venn <- "A:C"
      } else if (input$vennplaceT == paste(input$Patient2T, input$Patient3T, sep = ":")) {
        venn <- "B:C"
      } else if (input$vennplaceT == paste(input$Patient1T, input$Patient2T, input$Patient3T, sep = ":")) {
        venn <- "A:B:C"
      }
      if (venn != "") {
        cmd <- paste(cmd, "--VennPlace", venn)
