...
class ParsedVCF(pd.DataFrame):
    ...
    def duos(self, vcf2, VENNPLACE=None):
        ...
        difcols = [x.replace('_x','')
                   for x in mergedVCF.columns if '_x' in x]
        eqcols = [x for x in difcols if any(
            y in x for y in ['1000GP3', 'CLINVAR', 'ESP6500', 'RSID', 'POLYPHEN'])]
        difcols = set(difcols) - set(eqcols)
        # columnas a eliminar
        dropcols = [x + '_x'  for x in difcols]
        dropcols += [x + '_y' for x in difcols]
        dropcols += [x + '_y' for x in eqcols]
        dropcols += [x + '_x' for x in eqcols]
        ...
        for col in eqcols:
            mergedVCF[col] = mergedVCF[col + '_x'].combine_first(mergedVCF[col + '_y'])
        # combino las que podrían ser diferentes, si son diferentes (para variantes en ambos archivos, no las combino.
        for col in difcols:
            if all(tmp[col + '_x'] == tmp[col + '_y']):
                mergedVCF[col] = mergedVCF[col + '_x'].combine_first(mergedVCF[col + '_y'])
            else:
                dropcols.remove(col + '_x')
                dropcols.remove(col + '_y')
