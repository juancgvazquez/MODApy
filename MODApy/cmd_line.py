...
class Parser(object):
    ...
    def launcher(self):
        ...
        cmd = 'R -e shiny::runApp(\\"' + cfg.rootDir + \
            '/MODApy-Shiny.R\\"\\,port=3838\\,host=\\"0.0.0.0\\")'
        print(cmd)
        webapp = subprocess.Popen(shlex.split(
    ...
    def addPatient(self):
        parser = argparse.ArgumentParser(
            description="Downloads Patient Data to Patients folder. Receives both url or xls/xlsx")
        parser.add_argument(
            'FileorURL', help="URL to download or filepath to xls or xlsx files containing urls")
        args = parser.parse_args(argv[2:])
    ...
    def variantsDB(self):
        ...
        if args.addPatientToDB:
            logging.info('Reading Variants DB')
            ...
            logging.info(f'Adding Patient {patient}')
    ...
    def pipeline(self):
        ...
        parser.add_argument("-startStep", default=0, type=int,
                            help="Defines step to start running pipeline.")
        parser.add_argument("-endStep", default=0, type=int,
                            help="Defines step to start running pipeline.")
        ...
        if len(args.FQ) > 2:
            logger.error(
                'Only Two FASTQ files allowed. The Input for FastQ Files was: ', str(args.FQ))
            return exit(1)
            ...
            if args.keeptmp:
                newpipe.runpipeline(
                    fq1, fq2, keeptmp=True, startStep=args.startStep, endStep=args.endStep)
            else:
                newpipe.runpipeline(
            ...
            if args.keeptmp:
                ...
                newpipe.runpipeline(
                    fq1, startStep=args.startStep, endStep=args.endStep)
            return 0
    ...
    def single(self):
        ...
        try:
            ...
            logger.info("Running %s on patient %s" %
                        (str(args.Panel), str(args.Patient)))
            result = vcfmgr.ParsedVCF.from_vcf(patient).panel(panel)
            outpath = cfg.patientPath + result.name + '/Panels/' + \
                result.name + '_' + args.Panel + '.xlsx'
    ...
    def duos(self):
        ...
        parser.add_argument("--Panel", nargs='?', const=None,
                            help="Panel to run on Duos study")
        ...
        try:
            ...
            logger.info("Running Duos Study on %s and %s" %
